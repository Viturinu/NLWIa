...
async function main(){
    ...
    await prisma.prompt.create({
        data: {
            ...
            Abaixo você receberá uma transcrição desse vídeo, use essa transcrição para ferar os titulos.
            Abaixo você também receberá uma lista de títulos, use essa lista como referêmcia para os títulos a serem gerados.

            Os titulos devem ter no máximo 60 caracteres.
            Os titulos devem ser chamativos e atrativos para maximizar os cliques.
    ...
    await prisma.prompt.create({
        data: {
            ...
            Use palavras chamativas e que citavam a atenção de quem está lendo.
...
/*catch(error){ // duvida que tivemos
    console.error(error)
    await prisma.$disconnect()
    process.exit(1)
}
